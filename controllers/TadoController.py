...
class TadoController:
    ...
    def display_data_if_any(self, display: AutoDisplay):
        ...
        for data in self.tado_data:
            image_draw.text((10 + num * column_width, y_start), text=data.name, font=self.font)
            image_draw.text((10 + num * column_width, y_start + 50), text=data.current_temperature + "°C", font=self.large_font)
            image_draw.text((10 + num * column_width, y_start + 130), text=data.target_temperature + "°C min", font=self.font)
            num = num + 1
